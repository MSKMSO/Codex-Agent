...
The 16 per-user Claude bots (Aixa, Alejandro, Ashley, Axel, Cameron, David, Emily, Jesus, Jose, Lia, Neil, Neil-Claude, Rosi, Stephanie, Zahid, Afrah) all started returning **"Had trouble generating a reply"** around 08:30 PDT. Claude (the main Claude bot, internal service `mskai`), Yoo AI, and the three already-migrated bots (Heather, Kaye, Gabriel) were fine.
...
2. **Attempt "finish the per-user isolation migration"** — broke things. Twice. I now know:
   - **`/home/<user>/.claude` is a SYMLINK for 16 users, not a real directory.** `cp + chown + mv` chains followed the symlinks and rewrote the master file. The third user iteration's chown made the master `aixa:aixa`; then I broke it again the second time chowning to `zahid:zahid`. Each fix temporarily knocked Claude (mskai) offline.
