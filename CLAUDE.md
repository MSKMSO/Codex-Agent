...
**Important caution about the Tier 2 injection (lesson from 2026-05-14):** the auto-inject patcher must place the `DR_YOO_IDENTIFIERS_V2_VAULT BEGIN..END` block at **module-level scope** (column 0, after all function definitions), NOT inside an existing function's body. Pasting at column 0 in the middle of a function silently terminates that function at the dedent — Python parses the rest of the function body as dead code attached to the injected helper, and the original function returns None implicitly. **The mskai-responder.py incident on 2026-05-14 was caused by exactly this: the injection landed inside `run_codex`, killing the Claude call and causing every reply to come back as "Had trouble generating a reply" for ~6 hours.** Before any Tier 2 wiring, verify the responder file's structure — the BEGIN marker must come AFTER the last `def ` at column 0 in the file. After wiring, run a syntax check (`python3 -c "compile(open('<bot>-responder.py').read(),'x','exec')"`) AND restart the responder AND verify `responder-debug.log` mtime moves on a real DM (proves `run_codex` is being entered and writing its trace line). If any of those three checks fails, **revert the injection** before declaring the wiring done.

## When a bot says "Had trouble generating a reply" — read this first

If the user reports a bot replying with `⚠️ Had trouble generating a reply — please retry in a moment`, **do not guess**. Read [`docs/bot-empty-reply-diagnosis.md`](docs/bot-empty-reply-diagnosis.md) and run its decision tree in order. The three patterns (Anthropic rate limit / broken `run_codex` from code injection / Graph 404 red herring) all surface as the same message but require completely different (and incompatible) interventions. Doing the wrong intervention — most commonly cross-writing one bot's env file onto another's — silently merges credential identity and locks the receiver to the wrong AAD allow-list.
